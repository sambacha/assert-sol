...
function supportChangeTokenBalance(Assertion) {
  Assertion.addMethod('changeTokenBalance', function (token, signer, balanceChange) {
    ...
    var derivedPromise = Promise.all([getBalanceChangeForTransactionCall(subject, token, signer), getAddressOf(signer)]).then(function (_ref) {
      ...
      _this.assert(actualChange.eq(BigNumber.from(balanceChange)), "Expected \"" + address + "\" to change balance by " + balanceChange + " wei, " + ("but it has changed by " + actualChange + " wei"), "Expected \"" + address + "\" to not change balance by " + balanceChange + " wei,", balanceChange, actualChange);
    });
    this.then = derivedPromise.then.bind(derivedPromise);
    this["catch"] = derivedPromise["catch"].bind(derivedPromise);
    this.promise = derivedPromise;
...
export { chaiEthers };
//# sourceMappingURL=assert-evm.esm.js.map
